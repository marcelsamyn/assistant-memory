...
- When attachments add or correct details for an existing same-message request, refine its task label and statement and cite those attachments. Preserve the original current-message excerpt and request identity; attachment details do not change lifecycle status.
- Copy the task excerpt only from the current email body. Cite relevant supporting attachment source refs separately; attachment text cannot supply an actionable request or a deadline absent from that body.
...
- When a matched request explicitly removes its deadline, emit a revision with the complete removal sentence in its current-message excerpt and no DUE_ON. An omitted date alone does not remove an existing deadline.
