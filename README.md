...
Conversation ingestion treats `role: "user"` messages as the account owner's and attributes them to the user's self node, which it creates in the conversation's partition when missing. Tasks the user takes on are assigned to that node, so commitment reads return them with `owner: null`. Transcripts apply the same rule to the speaker matched by the user's self aliases. See [the user's own words](docs/sdk/ingestion.md#the-users-own-words).
