...
- Optionally supply known source provenance on documents and files: origin namespace, source kind, authorship/recipient relationship, chronology, linked sources, and completeness. The current contract also accepts `sourceKind: "message"` with required `messageId`, `threadId`, and `authoredAt`, and participants identified by email or `providerId`. Transcript ingestion accepts an optional origin `sourceKind`. Keep content in the document/file payload. Context cannot grant permissions or prove acceptance of a task.
...
- If queue acceptance fails after the receipt is stored, resubmit the same ingestion request with source-preserving defaults to restore its queued job. `retrySourceProcessing` can also restore queued jobs from retained conversion settings; older document sources may require the original request. Legacy `updateExisting: true` document replacements intentionally tombstone and recreate the source, so use the retry endpoint or replay with `updateExisting: false` when preserving the orphaned receipt matters.
- A revision includes content and meaningful extraction inputs. Corrected context can re-extract unchanged bytes and remove stale inferred requests while preserving explicit confirmations/dismissals. Display-only updates reuse processing. Omitted timestamps reuse the first stored value, including concurrent first ingestion. Retrying a known failed initial upload keeps the same file ID; an unknown upload outcome stays blocked for investigation.
- Source corrections also remove later inferred status and deadline claims that depend on the corrected request evidence. Manual decisions and independent evidence remain. Request-history prompts use a 32,000-character serialized window, with active and dismissed state first and an explicit omitted-record count; deterministic match validation keeps the complete history. Dutch bare “voor” or “tegen” dates stay undated unless the source supplies explicit deadline wording.
...
- Identity retirement also blocks conversation and transcript parent creation. Display-only document/file title replays refresh the linked Document label and embedding without extraction. Purged receipts keep their status and internal references but erase external identities and content hashes; clients can continue to use `getSourceProcessing` after purge.
...
- Link readable email attachments to the parent source with `parentSourceId`. Memory converts them, re-runs the parent email extraction with the attachment as supporting evidence, and keeps the parent message as the only source for actionable request and deadline citations. Late or corrected attachments refine existing request labels and statements without changing status or manual decisions. Incoming lifecycle updates require a known matching original requester; authenticated outgoing owner updates remain supported.
- Explicit removal wording in a matched email revision can clear an earlier inferred deadline. English and Dutch removal phrases are supported; omitted dates and unsupported replacements preserve the previous date. User-set dates remain intact. Render presentation excerpts with `presentation.source`, because their original source can differ from the latest task status source.
- Parent extraction uses attachment prefixes of at most 4,000 characters across the first 100 sources, within a shared 32,000-character serialized section limit. Truncated or omitted passages remain available through source reads. Processing completion does not mean every passage was examined. Refinements refresh canonical labels and search embeddings; identical file replays refresh Document labels from titles or filename fallbacks without another extraction.
...
- `MemoryClient.getSource({ userId, sourceId, includeContent: true })` now
  ...
  Conversion retains the original source payload separately; content reads still return converted Markdown when available.
