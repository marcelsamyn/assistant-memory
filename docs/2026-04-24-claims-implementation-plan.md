...
- SDK (`src/sdk/memory-client.ts`) exposes `bootstrapMemory`, `contextSearch` (covers personal + reference via `scope`), `getOpenCommitments`, `dedupSweep`, `ingestTranscript`, `setUserSelfAliases`, `cleanupPlaceholders`, plus pre-existing methods.
...
- **PR 4-ii**: transcript ingestion + speaker mapping + userSelfAliases. **Done.** Landed across two commits: (a) `b4ac6e1` `user_profiles.metadata.userSelfAliases` config with `POST /user/self-aliases` and SDK `setUserSelfAliases`; (b) `7a13dfe` `POST /transcript/ingest` with `segmentTranscript` LLM call (skipped on pre-segmented input), `resolveSpeakers` (user-self → knownParticipants → alias system → placeholder Person with `additionalData.unresolvedSpeaker=true` plus alias write-through), parent `meeting_transcript` source + child `conversation_message` sources with `metadata.speakerLabel`/`speakerNodeId`, and `extractGraph` `speakerMap` plumbing that emits `assertedByKind='participant'` + `assertedByNodeId` for non-self speakers. The pre-existing rejection of `participant` kind without a speaker map stays as a guard. Placeholder cleanup job deferred to PR 4-iii.
- **PR 4-iii**: full eval harness + observability + placeholder cleanup job. **Done.** Landed across three commits: (a) `7f5055d` `cleanupPlaceholders` job + `POST /maintenance/cleanup-placeholders` route surfacing aged unresolved-speaker Persons with optional `seedClaimsCleanupForPlaceholders` enqueue; (b) `36008eb` structured `logEvent` helper at `src/lib/observability/log.ts` with eight wired events (`claim.inserted` / `claim.superseded` / `claim.contradicted` / `claim.retracted` / `identity.resolved` / `atlas.derived` / `profile.synthesized` / `bootstrap_context.assembled` / `transcript.ingested`) plus a test sink for assertions; (c) `b86ede0` regression harness in `src/evals/memory/` exercising 11 stories deterministically (seeds the post-extraction graph state via direct DB writes per the plan's fallback guidance, then exercises lifecycle/cleanup/query helpers) plus a `threshold-calibration.ts` sub-harness sweeping identity thresholds across 10 curated label pairs. CLI: `pnpm run eval:memory`, `pnpm run eval:identity-thresholds`. Vitest gate: `RUN_EVALS=1`.
