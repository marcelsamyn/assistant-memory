...
### Cardinality gap: OWNED_BY / DUE_ON on Tasks (resolved in Phase 3 prerequisite)

`OWNED_BY` and `DUE_ON` on Task subjects need single-current-value lifecycle (a task has one current owner and one current due date; reassignment or rescheduling supersedes the prior claim) while the same predicates on non-Task subjects (atlas co-owners, shared artifacts) keep `multi_value` semantics. The predicate-policy registry now carries a `subjectTypeOverrides` axis: each entry can specify per-`NodeType` overrides for `cardinality` / `lifecycle`, and a single `resolvePredicatePolicy(predicate, subjectType)` resolver returns the effective policy. The lifecycle engine looks up each subject's `nodeType` and runs supersession only where the resolved policy is `single_current_value` + `supersede_previous`. The `getOpenCommitments` runtime newest-wins dedupe is now belt-and-suspenders against pre-backfill data; migration `0012_claims_task_owned_by_due_on_backfill.sql` recomputes the historical chain once at deploy.
