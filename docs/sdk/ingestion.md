...
A failed receipt can be retried while its queue job is retained, including a job completed after an unreadable conversion. Retry after fixing the converter or file problem; repeating the same unreadable bytes does not make them readable. A missing retained job returns a conflict rather than claiming recovery. Purged sources and unauthorized partitions cannot be revived by retry.
