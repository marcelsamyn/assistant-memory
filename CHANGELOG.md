...
- **`/query/timeline` period summaries** — pass `includePeriods: true` to receive a `periods` array of week/month/year temporal-rollup summaries alongside the day feed, bounded by the same `startDate`/`endDate`. Each entry is `{ key, granularity, summary, temporalNodeId }`, where `key` is the ISO period label (`2026` / `2026-06` / `2026-W24`), `granularity` is `"week" | "month" | "year"`, and `summary` is the rollup summary or `null` until the period has been summarized. Consumers can drop the separate `/query/graph` fetch for period summaries. Behavior is unchanged when the flag is omitted.
