...
- **Contextual source ingestion and durable processing receipts.** `ingestDocument` and `ingestFile` accept an optional, versioned `sourceContext` that describes application-owned source facts such as mailbox identity, participant roles, message chronology, and attachment relationships. Accepted document and file revisions return `ingestionOperationId`; `MemoryClient.getSourceProcessing` reports whether that exact revision is queued, processing, completed, failed, or purged, and `retrySourceProcessing` restarts a retained failed operation. `sourceIdentityLifecycle` closes a durable stable-identity gate before an external account is removed and reopens it when the account is restored. Existing callers can omit the new context and continue to use `jobId` and `sourceId`.

- **Tentative email requests and lifecycle evidence.** Email extraction now creates candidates only for direct requests addressed to the authenticated user and explicit promises written by that user. It preserves per-request identity across a thread, keeps same-titled requests separate, and applies cited clarification, completion, and material-revision evidence without treating receipt or a reply as acceptance. CC-only mail, newsletters, auto-replies, quoted history, and attachments cannot create tasks by themselves.
...
- **Extracted tasks the user takes on are assigned to the user's self node.** Conversation extraction used to identify the user only by name, so a task's `ASSIGNED_TO` could land on a same-named Person, and commitment reads showed that Person as the owner. Conversation ingestion now resolves the flagged self node in the source's partition and gives the model its ID as the author of `role: "user"` messages. The transcript prompt tells the model to use the user-self speaker's node as the `ASSIGNED_TO` object as well. The dedup sweep no longer merges the self node, which let an older same-named Person absorb it and drop its `isUserSelf` flag. A new self node is labeled "Me" until the user's aliases name them; it used to carry the raw user ID. Existing assignments and labels are not rewritten.
